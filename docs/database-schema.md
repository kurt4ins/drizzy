...
Нормализованное хранилище для пользователей, профилей, предпочтений, взаимодействий, мэтчей, поведенческих агрегатов, рейтингов и рефералов. Миграции управляются через [goose](https://github.com/pressly/goose).
...
| ------------------- | ----------------- | ----------------------------------------------- |
| `user_id`           | BIGINT            | PK, FK → `users(id)`                           |
| `score`             | DOUBLE PRECISION  | Итоговая оценка кандидата (алгоритм TBD)        |
| `algorithm_version` | TEXT              | напр. `v1.0.0`                                  |
| `recalculated_at`   | TIMESTAMPTZ       | NOT NULL, default `now()`                       |

**Индексы:** `(score DESC)` для ранжированных запросов.

> **TODO:** алгоритм ранжирования не определён. Таблица служит плейсхолдером — asynq-воркер пишет `score`, discovery refill читает его при заполнении очереди.
